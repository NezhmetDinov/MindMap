(function(b) {
  ...
  b(document).ready(function() {
    function q(a) {
      if (!(1E3 > +new Date - l)) return l = +new Date, d.execute(b("#textArea").val(), a), e(), !1
    }
    ...
    function r(a) {
      if (new Date - v < z) setTimeout(r, z);
        ...
        B = !1;
        a = a ? b(window).innerWidth() -
          20 : b(window).innerWidth() - b("#leftPane").width() - 20;
        var g = b("#containerTable").height();
        d.settings({
          width: a,
          height: g
        })
    ...
    function n() {
      b("#logo i").hasClass("icon-chevron-down") ? (b("#logo i").removeClass("icon-chevron-down").addClass("icon-chevron-up"), b("#leftPane").hide()) : (b("#logo i").removeClass("icon-chevron-up").addClass("icon-chevron-down"), b("#leftPane").show());
      r(!0)
    }
    ...
    function e() {
      ...
      for (var c = d.settings("bgcolors"), h = 0; h < a; h++) {
        ...
        var e = 0 == h ? "Root" : g + " " + h;
        b("#colorsdiv").append("<span class=s50>" + e + "</span>").append("<input class='bgcolors' id='color" + h + "' pickerNbr=" + h + " data-default-value=" + c[h] + " type=text value='" + c[h] + "'><br>")
      }
      b(".bgcolors").minicolors({
        textfield: !1,
        change: function(a, b) {
    ...
    function p() {
      setCookie();
      b("saveModal").modal("hide");
      b(".errorMsg").hide();
      b(".loginUser").hide();
      b("#sendMsg").html("");
      logged_in && userid ? (b("#mapEmail").val(user), b("#user").text(user), b(".guest").hide(), b(".user").show()) : (b(".user").hide(), b(".guest").show());
      if ("unsaved" == savingState) {
        var a = b("#textArea").val();
        (a = a.substring(0, a.indexOf("\n"))) && b("#mapTitle").val(a.replace("\t", ""));
        b("#savingStateSaved").hide();
        b("#savingStateViewOnly").hide();
        b("#savingStateUnsaved").show()
      } else if ("viewonly" == savingState) b("#savingStateSaved").hide(), b("#savingStateViewOnly").show(), b("#savingStateUnsaved").show();
      else if ("saved" == savingState) b("#savingStateViewOnly").hide(), b("#savingStateUnsaved").hide(), b("#savingStateSaved").show(), w();
      else {
    ...
    function w() {
      if ("unsaved" == savingState || "viewonly" == savingState) {
        var a = b("#mapTitle").val(),
          g = a && 0 < a.length && " " != a && "  " != a;
        b("#reminderForT")[0].style.display =
          g ? "none" : "inline";
        var c = b("#mapEmail").val(),
          h = c && 2 == c.split("@").length && c.lastIndexOf(".") > c.indexOf("@");
        b("#reminderForE")[0].style.display = h ? "none" : "inline";
        if (!g || !h) return !1
      }
      b(".savingInProgress").show();
      b("#savingFinished").hide();
      g = C();
      a && (g.maptitle = a);
      c && (g.email = c);
      ...
      b.ajax({
        ...
        success: function(a) {
          b(".errorMsg").hide();
          b("#savingFinished").show();
          b(".savingInProgress").hide();
          b("#saveMindMapBtn").hide();
          a.status && "OK" == a.status ? (savingState = "saved", admincode = a.admincode, viewcode = a.viewcode, b("#savingStateUnsaved").css("display", "none"), b("#savingStateViewOnly").css("display", "none"), b("#savingStateSaved").css("display", "inline"), b(".savedMapInfo").show(), url = "text2mindmap.com/" + a.admincode, b(".adminLink").html('<a href="http://' + url + '" target="_blank" title="Link to this mind map">' + url + "</a>"), url = "text2mindmap.com/" + a.viewcode, b(".viewLink").html('<a href="http://' +
            url + '" target="_blank" title="Link to a view-only version of this mind map">' + url + "</a>"), b("#lastSavedInfo").html(a.lastsaved), hasChanged = !1, b(".saveMsg").hide()) : (console.log("handleSaveResponse(): data.status did not return OK"), y())
        },
        timeout: 3E4,
        error: function(a) {
          y(a)
    ...
    function A() {
      d.getImage(function(a) {
        s = a;
        b("#thumbnailImage")[0].src = s
      }, 1);
      b("#downloadPNG").html('<i class="icon-download-alt"></i> Download image</button>');
      b("#downloadPDF").html('<i class="icon-download-alt"></i> Download PDF</button>');
      b("#downloadModal").modal();
      return !1
    }
    ...
    function D(a, g) {
      if (1 == g) {
        var c = s;
        b('<form method="POST" action="' + a + '" style="display:none"><textarea name="imgdata" style="display:none">' + c + "</textarea></form>").appendTo("body").submit();
        b("#downloadPNG").html('<i class="icon-download-alt"></i> Download image')
      } else d.getImage(function(g) {
        c = g;
        b('<form method="POST" action="' + a + '" style="display:none"><textarea name="imgdata" style="display:none">' +
          c + '</textarea><input type="hidden" name="logged_in" value="' + logged_in + '"></form>').appendTo("body").submit()
      }, 2), b("#downloadPDF").html('<i class="icon-download-alt"></i> Download PDF')
    }

    function C() {
      var a = {};
      a.text = b("#textArea").val();
    ...
    function m(a) {
      try {
        ...
        b(".fontcolor").minicolors("value", g.fontColor);
        b(".linecolor").minicolors("value", g.lineColor);
        var c = 1 == g.lockAfterMoving ? !0 : !1;
        b("#lockAfterMoving").prop("checked", c);
        1 == g.coloringMode ? (b("#mode1").addClass("active"), b("#mode2").removeClass("active")) : (b("#mode1").removeClass("active"), b("#mode2").addClass("active"));
        d.settings(g);
        q(!0);
    ...
    var x = b(window).innerWidth() - b("#leftPane").width() - 20,
      f = b(window).innerHeight() - 18,
      d = new I("stageHolder", {
        width: x,
        height: f
      });
    ...
    var s = null,
      t = "",
      l = 0;
    b.support.opacity || b("#upgradeModal").modal();
    b("#textArea").keydown(function(a) {
    ...
    b(window).resize(function() {
      v = new Date;
      !1 === B && (B = !0, setTimeout(r, z))
    });
    b("#indent").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b("#textArea").trigger(jQuery.Event("keydown", {
        which: 9,
        shiftKey: !1
      }))
    });
    b("#outdent").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b("#textArea").trigger(jQuery.Event("keydown", {
        which: 9,
    ...
    b("#logo").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      n()
    });
    b("[rel='tooltip']").tooltip();
    b("#showFaq").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b("#faqMore").show();
      b(this).hide();
    ...
    b("#zoomInBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      d.zoom(120);
      return !1
    });
    b("#zoomOutBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      d.zoom(-120);
      return !1
    });
    b("#lockAfterMoving").change(function() {
      d.settings({
        lockAfterMoving: b(this).prop("checked")
      })
    });
    b(".coloringMode").on("touchstart click", function(a) {
      (a = b(this).attr("coloringMode")) || (a = 1);
      d.settings({
        coloringMode: a
      });
      e();
      b(".coloringMode").removeClass("active");
      b(this).addClass("active")
    });
    b("a.fonts").on("touchstart click", function(a) {
      d.settings({
        font: b(this).text()
      })
    });
    b("a.fontSize").on("mouseover touchstart", function() {
      d.settings({
        fontSize: b(this).text()
      })
    });
    b(".lineWidth").on("mouseover touchstart", function() {
      d.settings({
        lineWidth: b(this).attr("val")
      })
    });
    b("input.fontcolor").minicolors({
      textfield: !1,
      animationSpeed: 0,
      change: function(a, b) {
        d.settings({
    ...
    b("#mapEmail").keydown(function(a) {
      13 == a.which && w()
    });
    b("#signInFromModal").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b(".user, .guest").hide();
      b(".loginUser").show();
    ...
    b("#signInBtn").on("touchstart click", function(a) {
      setCookie()
    });
    b("#signOutFromModal").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b.get("index.php/user/signout/0", function() {
    ...
    b("#signInFromModalForm").submit(function(a) {
      b("#signInMsg").hide();
      a.preventDefault();
      a = b("#signInFromModalForm .user_username").val();
      var g = b("#signInFromModalForm .user_password").val();
      a && g ? b.ajax({
        type: "POST",
        url: "index.php/user/signin",
        data: b("#signInFromModalForm").serialize(),
        success: function(a) {
          "0" != a && 0 != a && a ? (a = JSON.parse(a), userid = a.userid, user = a.user,
            logged_in = 1, p()) : b("#signInMsg").text("Sorry, wrong email/password combination").show()
        },
        error: function() {
          b("#signInMsg").text("Sorry, there seems to be a technical problem.").show()
        }
      }) : b("#signInMsg").text("Please provide your email and password to sign in").show();
      return !1
    });
    b("#saveAsGuestLink").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b(".loginUser, .user").hide();
      b(".guest").show()
    });
    b("#saveBtn, .saveBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      p();
      return !1
    });
    b("#saveMindMapBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      w();
      return !1
    });
    b("#lockAll").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      d.setNodeLocks(!0);
      return !1
    });
    b("#unlockAll").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      d.setNodeLocks(!1);
      return !1
    });
    b("#helpBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b("#introModal").modal();
      return !1
    });
    b("#newBtn, .newBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b(".modal").modal("hide");
      hasChanged && !1 == confirm("Create a new mind map without saving this one first?") ? a = void 0 : (b("#textArea").val(""), d.clear(), savingState = "unsaved", viewcode = admincode = "", hasChanged = !1, b(".saveMsg").hide(), b.cookie("current-mindmap", !1, {
        expires: -1
      }), a = !1);
      return a
    });
    b("#downloadBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      return A()
    });
    b("#downloadPNG").on("touchstart click",
      function(a) {
        a.stopPropagation();
        a.preventDefault();
        b("#downloadPNG").append(" <i class='icon-spinner icon-spinn'></i>");
        D("downloadPNG.php", 1)
      });
    b("#downloadPDF").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b("#downloadPDF").append(" <i class='icon-spinner icon-spinn'></i>");
      D("downloadPDF.php", 2)
    });
    b("#feedbackBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      b("#sendFeedbackMsg").html("Sending... <i class='icon-spinner icon-spin'></i>");
      b.ajax({
        ...
        timeout: 3E4,
        success: function() {
          b("#sendFeedbackMsg").html("<b class='text-success'>Sent. Thank you for your feedback.</b>")
        }
      });
      return !1
    });
    b("#sendMailBtn").on("touchstart click", function(a) {
      b("#sendMsg").html("Sending... <i class='icon-spinner icon-spin'></i>");
      a.stopPropagation();
      a.preventDefault();
      b.ajax({
        url: "sendMail.php",
        data: {
          address: b("#sendEmailMail").val(),
          admincode: admincode,
          viewcode: viewcode
        },
        timeout: 3E4,
        success: function() {
          b("#sendMsg").html(" <b class='text-success'> Sent</b>")
        },
        error: function() {
          b("#sendMsg").html(" Sorry, there seem to be a technical problem. Please try again later.")
        }
      });
      return !1
    });
    b("#sendReminderBtn").on("touchstart click", function(a) {
      b("#sendReminderMsg").html("Sending... <i class='icon-spinner icon-spin'></i>");
      a.stopPropagation();
      a.preventDefault();
      b.ajax({
        url: "sendReminder.php",
        data: {
          email: b("#forgot_e").val()
        },
        timeout: 3E4,
        success: function() {
          b("#sendReminderMsg").html("<b class='text-success'>Email sent.</b>")
        },
        error: function() {
          b("#sendReminderMsg").html(" Error: Please try again later.")
        }
      });
      return !1
    });
    b("#toggle1").on("touchstart click", function(a) {
      b("#toggle1").removeClass("toggleInactive").addClass("toggleActive");
      b("#toggle2").removeClass("toggleActive").addClass("toggleInactive");
      b("#tab2").removeClass("active");
      b("#tab1").addClass("active");
      a.preventDefault()
    });
    b("#toggle2").on("touchstart click", function(a) {
      b("#toggle2").removeClass("toggleInactive").addClass("toggleActive");
      b("#toggle1").removeClass("toggleActive").addClass("toggleInactive");
      b("#tab1").removeClass("active");
      b("#tab2").addClass("active");
      a.preventDefault()
    });
    b("#drawBtn").on("touchstart click", function(a) {
      a.stopPropagation();
      a.preventDefault();
      q();
      var g;
      b("#textArea").val() == t ? (b("#holder").popover({
        ...
        delay: {
          ...
          b("#holder").popover("destroy")
        },
        2200)) : t = b("#textArea").val()
    });
    x = b("#textArea").val().indexOf("\n", b("#textArea").val().indexOf("\n") + 1);
    this.setSelectionRange && b("#textArea")[0].setSelectionRange(x, x);
    setCookie = function() {
      if ("error404" != viewcode) {
        var a = C();
        a && (b.cookie("current-mindmap", JSON.stringify(a), {
          expires: 1
        }), b(".saveMsg").show());
        a = new Date;
        a.setTime(a.getTime() + 6E5);
        b.cookie("timestamp", "1", {
          expires: a
        })
      }
    };
    map ? (m(map), b("#lastSavedInfo").html(map.lastsaved), b(".savedMapInfo").show()) : (f = b.cookie("current-mindmap"), x =
      b.cookie("timestamp"), f ? (f = JSON.parse(f)) && f.text && "" != f.text ? (m(f), x || b("#welcomeBackModal").modal()) : q(!0) : (x || b("#introModal").modal(), q(!0)))
  });
  var I = function(q, r) {
    function n(a, g, c) {
    ...
    z.on("touchstart mousedown", function(a) {
      m || (x = !0, b(window).on("touchmove mousemove", function(a) {
        a.stopPropagation();
        a.preventDefault();
    ...
    b(window).on("mouseup touchend", function(a) {
      x && (b(window).unbind("touchmove mousemove"), x = !1);
    ...
    return that = {
      ...
      execute: function(a, g) {
        f && f.stop();
        that.text2mindmap(a);
        !0 !== g && (hasChanged = !0, b(".saveMsg").show());
      ...
      newText: function(a) {
        ...
        g.on("mouseover", function() {
          ...
          for (var c = d.nodes.indexOf(a), g = b("#textArea").val().split(/\n/), e = -1, f = 0; f <= c; f++) e += g[f].length + 1; -
          1 != e && "undefined" != typeof e && (f = b("#textArea")[0], g = e, f.setSelectionRange ? (f.focus(), f.setSelectionRange(g, e)) : f.createTextRange && (f = f.createTextRange(), f.collapse(!0), f.moveEnd("character", e), f.moveStart("character", g), f.select()), e = b("#textArea"), g = parseInt(e.css("line-height")), e.scrollTop(c * g))
        });
